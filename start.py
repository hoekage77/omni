...
def print_manual_instructions():
    """Prints instructions for manually starting Agent X services."""
    print(f"\n{Colors.BLUE}{Colors.BOLD}🚀 Manual Startup Instructions{Colors.ENDC}\n")

    print("To start Agent X, you need to run these commands in separate terminals:\n")
    ...
    print("Once all services are running, access Agent X at: http://localhost:3000\n")
...
def main():
    ...
    if "--help" in sys.argv:
        ...
        print("Manage Agent X services based on your setup method")
        print("\nOptions:")
    ...
    if setup_method == "manual":
        ...
        print("Managing all Agent X services with Docker Compose...\n")
        ...
        if is_up:
            action = "stop"
            msg = "🛑 Stop all Agent X services? [y/N] "
        else:
            action = "start"
            msg = "⚡ Start all Agent X services? [Y/n] "
        ...
        if action == "stop":
            subprocess.run(["docker", "compose", "down"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Agent X services stopped.{Colors.ENDC}")
        else:
            subprocess.run(["docker", "compose", "up", "-d"], shell=IS_WINDOWS)
            print(f"\n{Colors.GREEN}✅ All Agent X services started.{Colors.ENDC}")
            print(f"{Colors.CYAN}🌐 Access Agent X at: http://localhost:3000{Colors.ENDC}")
