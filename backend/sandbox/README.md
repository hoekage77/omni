...
2. Build a custom image:
   ...
   docker push agentx/agent-x:0.1.3
   ```
...
1. Change the `image` parameter in `docker-compose.yml` (that defines the image name `agentx/agent-x:___`)
...
3. Push the new version: `docker push agentx/agent-x:0.1.3`
