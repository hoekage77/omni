...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | Agent X`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by Agent X`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Agent X`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData) {
      const hasSeenUpgradeDialog = localStorage.getItem('agentx_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    ...
    localStorage.setItem('agentx_upgrade_dialog_displayed', 'true');
  };
